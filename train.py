...
train_dataset = train_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         ...
                         Tout=[tf.int32, tf.float32, tf.float32, tf.float32, tf.float32]),  # tf.int64 Linux下
...
val_dataset = val_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         ...
                         Tout=[tf.int32, tf.float32, tf.float32, tf.float32, tf.float32]), #int64 Linux下
    num_parallel_calls=args.num_threads
)
val_dataset.prefetch(args.prefetech_buffer)

# 参考https://blog.csdn.net/briblue/article/details/80962728
