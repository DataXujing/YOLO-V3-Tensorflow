...
# yolo v3 前向传播，损失函数的计算，其中损失函数的计算同YOLO V1，YOLO v2
...
class yolov3(object):
    ...
    def reorg_layer(self, feature_map, anchors):
        '''
        这部分是看你对numpy的运用和高维数组的运用了！
        三个featuremap，其中一个feature map
    ...
    def predict(self, feature_maps):
        ...
        for result in reorg_results:
            ...
            # 与YOLO V2不同的地方 softmax替换成了sigmoid
        ...
        # shape: [N, (13*13+26*26+52*52)*3, 4] 所有Box放在一起
        boxes = tf.concat(boxes_list, axis=1)
        # shape: [N, (13*13+26*26+52*52)*3, 1] 所有confs放在一起
        confs = tf.concat(confs_list, axis=1)
        # shape: [N, (13*13+26*26+52*52)*3, class_num] 所有class_num放在一起
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        # [N, 13, 13, 3, 1]
        object_mask = y_true[..., 4:5]  # confidence 目的在后面保留confidence是1的下标
        # shape: [N, 13, 13, 3, 4] & [N, 13, 13, 3] ==> [V, 4]
        # V: num of true gt box  gt的个数
        # 在做目标检测（YOLO）时涉及到一个函数boolean_mask(a,b) 将使a (m维)矩阵仅保留与b中“True”元素同下标的部分。
    ...
    def broadcast_iou(self, true_box_xy, true_box_wh, pred_box_xy, pred_box_wh):
        '''
        maintain an efficient way to calculate the ious matrix between ground truth true boxes and the predicted boxes
